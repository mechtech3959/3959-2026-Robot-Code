...
public class LEDHandler {
        ...
        public LEDHandler(LEDSubsystem ledSubsystem, 
                ...
                neitherIntakingNorShootingTrigger = new Trigger(
                                () -> (shooterSubsystem.getShooterState() == ShooterSubsystem.ShooterStates.REST)
                                                && (intakeSubsystem.getState() != IntakeSubsystem.IntakeStates.INTAKE));
                headingLockTrigger = new Trigger(
                                () -> drivetrainSubsystem.getSwerveState() == DrivetrainSubsystem.SwerveStates.Heading);
           teliDriveTrigger = new Trigger(
                                () -> drivetrainSubsystem.getSwerveState() == DrivetrainSubsystem.SwerveStates.TeleOp);
        ...
        public void handleLEDs() {
                ...
                match_EndTrigger
                                .onTrue(Commands.runOnce(() -> ledSubsystem
                                                .changeMainLEDState(LEDSubsystem.MainLEDStates.MATCH_END)));
                RobotModeTriggers.disabled().and(match_EndTrigger.negate())
                                .onTrue(Commands.runOnce(() -> ledSubsystem
                                                .changeMainLEDState(LEDSubsystem.MainLEDStates.DISABLED)));
                RobotModeTriggers.autonomous()
                                .onTrue(Commands.runOnce(() -> ledSubsystem
                                                .changeMainLEDState(LEDSubsystem.MainLEDStates.AUTO)));
                RobotModeTriggers.teleop()
                                .onTrue(Commands.runOnce(() -> ledSubsystem
                                                .changeMainLEDState(LEDSubsystem.MainLEDStates.TELEOP)));
                intakingTrigger.and(shootingTrigger.negate())
                                .onTrue(Commands.runOnce(() -> ledSubsystem.changeSecondSecondaryLEDState(
                                                LEDSubsystem.SecondSecondaryLEDStates.INTAKING)));
                shootingTrigger.and(intakingTrigger.negate())
                                .onTrue(Commands.runOnce(() -> ledSubsystem.changeSecondSecondaryLEDState(
                                                LEDSubsystem.SecondSecondaryLEDStates.SHOOTING)));
                neitherIntakingNorShootingTrigger
                                .onTrue(Commands.runOnce(() -> ledSubsystem.changeSecondSecondaryLEDState(
                                                LEDSubsystem.SecondSecondaryLEDStates.NEITHER)));
                intakingAndShootingTrigger
                                .onTrue(Commands.runOnce(() -> ledSubsystem.changeSecondSecondaryLEDState(
                                                LEDSubsystem.SecondSecondaryLEDStates.INTAKING_AND_SHOOTING)));
                headingLockTrigger
                                .onTrue(Commands.runOnce(() -> ledSubsystem.changeSecondaryLEDState(
                                                LEDSubsystem.SecondaryLEDStates.HEADING_LOCK)));
                teliDriveTrigger.onTrue(Commands.runOnce(() -> ledSubsystem
                                .changeSecondaryLEDState(LEDSubsystem.SecondaryLEDStates.MATCH_READY)));
